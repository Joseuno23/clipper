...
3. Keep `vercel.json` and `.vercelignore` committed so Vercel builds only the Vite static frontend and ignores the root `api/` handlers. The API runs on Railway, not Vercel Functions.
