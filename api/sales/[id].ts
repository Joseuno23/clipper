...
import { parseSaleUpdateInput } from "../../src/server/domain/sales/validation";
...
export default createApiHandler({
  ...
  async handle(request, _response, { method }) {
    ...
    if (method === "GET") return saleService.get(context, id);

    requireAdminCapable(context);
    return saleService.update(
      context,
      id,
      parseSaleUpdateInput(parseJsonBody(request)),
    );
  },
});
